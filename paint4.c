...
typedef struct {
    ...
    char pen;
		...
		//
		//
		int color; // 色：default, red, blue, green, yellow, 
		//
		//
} Canvas;
...
//
//
typedef enum res{ EXIT, LINE, RECT, CIRCLE, LOAD, ERROR, CHPEN, CHCOLOR,UNDO, SAVE, UNKNOWN, ERRNONINT, ERRLACKARGS, NOCOMMAND} Result;
//
//
...
//
//
void chcolor(Canvas *c, History *his, const int *num);
//
//
...
Canvas *init_canvas(int width,int height, char pen, int color)
{
    ...
    new->canvas = (char **)malloc(width * sizeof(char *));
		//
		//
		new->canvas2 = (int **)malloc(width * sizeof(int *));
		//
		//
    ...
    memset(tmp2, 39, width*height*sizeof(int));
    for (int i = 0 ; i < width ; i++){
	new->canvas2[i] = tmp2 + i * height;
    }
		//
		//
...
void reset_canvas(Canvas *c)
{
    ...
    memset(c->canvas[0], ' ', width*height*sizeof(char));
		//
		//
		memset(c->canvas2[0], ' ', width*height*sizeof(int));
		//
		//
}
...
void print_canvas(Canvas *c)
{
    ...
    char **canvas = c->canvas;
		//
		//
		int **canvas2 = c->canvas2;
		const int color = c->color;
		//
		//
	...
	for (int x = 0 ; x < width; x++){
	    const char c = canvas[x][y];
			//
			//
			const char c2 = canvas2[x][y];
			char k[5];
			snprintf(k, sizeof(k), "%d", c2);
			printf("\x1b[%sm%c\x1b[0m", k, c);
	    // printf("%c", c);
			//
			//
...
void free_canvas(Canvas *c)
{
    ...
    free(c);
		//
		//
    free(c->canvas2[0]); //  for 2-D array free
    free(c->canvas2);
    free(c);
		//
		//
...
Result interpret_command(const char *command, History *his, Canvas *c)
{
    ...
    if (strcmp(s, "chcolor") == 0) {
	    s = strtok(NULL, " "); // 空白、ごとに区切って入力コマンドを読み取る！！
			int k;
	    if (s[0] == 0){
				s = "default";
	    }
			if (s[0] == 'r' && s[1] == 'e' && s[2] == 'd' && s[3] == '\0') {
				...
				k = 33;
			} else if (s[0] == 'b' && s[1] == 'l' && s[2] == 'u' && s[3] == 'e' && s[4] == '\0') {
				k = 34;
			} else {
				k = 39;
			}
